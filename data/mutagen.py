...
class Mutagen(InMemoryDataset):
    ...
    def process(self):
        ...
        for i in range(original_labels.shape[0]): # i is the label of graphs
            ...
            if len(signal_nodes) != 0:
                ...
                motif_type = set(node_type[signal_nodes].tolist())
                #assert motif_type in ({4, 1}, {4, 3}, {4, 1, 3})  # NO or NH
                if motif_type == {4,1}: # NO
                    motif_type = 1
                elif motif_type == {4,3}: # NH
                    motif_type = 2
                else:                   # {1,3,4} NO && NH
                    motif_type = 3
            else:
            ...
            if y.item() == 0 and len(signal_nodes) == 0:  # filter mutagen without NO2 or NH2
                continue
